  ...
  <Accordion title="Child Node Disconnected (Network Issues)" icon="link-slash">
    ...
    <Steps>
      ...
      <Step title="Validate API Key Configuration">
        Worker nodes must provide a valid API key to connect. Check the child node's environment:
        ```bash
        docker compose exec backend env | grep -E "MAIN_NODE_URL|NODE_API_KEY"
        ...
        Get the API key from the main node:
        ```bash
        # On main node
        docker compose exec backend env | grep NODE_API_KEY
        ```

        If missing, add `NODE_API_KEY` to the child node's `docker-compose.yml` environment section and restart:
        ```bash
        docker compose restart backend
        ```

        <Note>
        `NODE_API_KEY` must match the value set on the main node. Generate a secure key with `openssl rand -hex 32` and set it on both nodes.
        </Note>
      </Step>
    </Steps>
  ...
  <Accordion title="Frontend Can't Connect to Backend" icon="browser">
    ...
    <Steps>
      ...
      <Step title="Check API_URL Configuration">
        ...
        # Should be:
        # API_URL=http://backend:3100  (internal Docker network)
        # OR
  ...
  <Accordion title="Out of Disk Space During Encoding" icon="hard-drive">
    ...
    <Steps>
      ...
      <Step title="Clear Old Temporary Files">
        ...
        docker compose exec backend rm -rf /tmp/bitbonsai/*
      ...
      <Step title="Enable Two-Pass Encoding (Smaller Temps)">
        ...
        - Higher CRF values (e.g., CRF 23 instead of 18) — higher CRF = smaller output file
...
| FFmpeg Output | Check job details in UI (Encoding tab → click job → expand logs) |

<Tip>Add `-f` flag to follow logs in real-time: `docker compose logs -f backend`</Tip>

## Recovery Procedures

### Full System Reset (Nuclear Option)

<Warning>This will delete all job history and settings. Use only as last resort.</Warning>
...
docker compose logs backend | grep "Application listening"
```
...
<Card
  ...
  icon="github"
  href="https://github.com/bitbonsai/bitbonsai/issues/new"
>
...
4. Full error logs (use `docker compose logs --tail=100 backend`)
...
# Note: use the actual container name (bitbonsai-backend) for --net=container, not the service key
```

## Next Steps

<CardGroup cols={2}>
  <Card title="Dashboard & Monitoring" icon="chart-line" href="/guides/monitoring">
    Track job health, queue status, and system metrics
  </Card>
