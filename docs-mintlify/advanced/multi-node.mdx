...
Install the NFS client on every node before mounting:
...
services:
  ...
  backend:
    ...
    ports:
      - "3100:3100"
    environment:
      DATABASE_URL: postgresql://bitbonsai:YOUR_SECURE_PASSWORD@postgres:5432/bitbonsai
      NODE_MODE: MAIN
      CONCURRENT_JOBS: 4
    volumes:
      - /mnt/videos:/media
...
services:
  backend:
    ...
    ports:
      - "3100:3100"
    environment:
      DATABASE_URL: postgresql://bitbonsai:YOUR_SECURE_PASSWORD@192.168.1.100:5432/bitbonsai
      NODE_MODE: LINKED
...
Replace `YOUR_SECURE_PASSWORD` with the same password used on the main node postgres service.
...
- **[Distributed Encoding Architecture](/advanced/distributed-encoding)** - How shared-database polling works across nodes
