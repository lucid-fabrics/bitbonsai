...
Scale BitBonsai's transcoding capacity by adding worker nodes across multiple machines. Distribute encoding jobs automatically with zero manual intervention.
...
- **Shared Storage**: NFS mount ensures all nodes access the same video files (zero-copy encoding)
...
```mermaid
graph TD
    UI[Web UI :4210] -->|API calls| Main[Main Node :3100]
    Main -->|owns| DB[(PostgreSQL)]
    Main -->|reads/writes| NFS[NFS Share /videos]

    Main -->|distributes jobs| Main
    Main -->|distributes jobs| Child1[Child Node 1]
    Main -->|distributes jobs| Child2[Child Node 2]

    Child1 -->|proxy API| Main
    Child2 -->|proxy API| Main

    Child1 -->|reads/writes| NFS
    Child2 -->|reads/writes| NFS
...
|-----------|------|------------------|
| **Main Node** | Master + Worker | Database, API, UI, job distribution, encoding |
| **Child Node** | Worker only | Fetch jobs, encode, report progress, proxy API calls |
| **NFS Share** | Shared storage | Centralized video file access (no file copying) |
| **PostgreSQL** | State management | Job queue, libraries, node registry |
...
<Steps>
  ...
  <Step title="Navigate to Nodes Page">
    Click **Settings** → **Nodes** in the sidebar.
  </Step>
...
<Steps>
  <Step title="Open Nodes Page">
    Navigate to **Settings** → **Nodes** in the main node's UI.
  </Step>
...
```
Available Nodes:
  - Main Node:  2/4 jobs  → Load = 0.50
  - Child Node: 3/8 jobs  → Load = 0.375
