...
Choose the right [codec](/glossary#codec) for your [library](/glossary#library) based on [encoding](/glossary#encoding) speed, file size, and device compatibility.

<Note>
  **New to video encoding?** A codec is the format used to compress video. See the [codec glossary
  entry](/glossary#codec) for a simple explanation.
</Note>

| Feature                                                      | [HEVC](/glossary#codec) (H.265) | AV1                      |
| ------------------------------------------------------------ | ------------------------------- | ------------------------ |
| **File Size Reduction**                                      | 40-60% smaller vs H.264         | 50-70% smaller vs H.264  |
| **Encoding Speed**                                           | Fast (CPU/[GPU](/glossary#gpu)) | Slow (CPU only)          |
| **[Hardware Acceleration](/glossary#hardware-acceleration)** | NVIDIA, Intel, AMD              | Limited (Intel Arc only) |
| **Device Compatibility**                                     | Widespread (2015+)              | Limited (2020+)          |
| **Quality at Same [Bitrate](/glossary#bitrate)**             | Good                            | Excellent                |
...
<Info>
  **TL;DR:** Use **HEVC** for faster encoding and broad device support. Use **AV1** for maximum
  space savings if you have time and modern playback devices.
</Info>
...
<Card title="Fast Encoding Required" icon="bolt">
  Encoding large libraries (1000+ movies) where time matters more than maximum compression. HEVC
  encodes 5-20x faster than AV1.
</Card>

<Card title="Hardware Encoder Available" icon="microchip">
  NVIDIA RTX 20/30/40 series, Intel 6th gen+, or AMD RX 6000+ [GPUs](/glossary#gpu) provide 3-10x
  speedup with NVENC/QSV/VCE [hardware acceleration](/glossary#hardware-acceleration).
</Card>

<Card title="Apple Ecosystem" icon="apple">
  iPhone, iPad, Mac, Apple TV all support HEVC natively since 2016. Safari requires HEVC for
  efficient video streaming.
</Card>

<Card title="Smart TV Playback" icon="tv">
  Most smart TVs from 2016+ support HEVC hardware decoding. Older or budget TVs often lack AV1
  support.
</Card>
...
<Tip>
  Enable [hardware acceleration](/glossary#hardware-acceleration) in **Settings → Encoding** to
  unlock [GPU](/glossary#gpu) speeds. [HEVC](/glossary#codec) on NVIDIA RTX 3060+ provides
  near-identical quality to CPU at 5-10x the speed.
</Tip>
...
<Check>
  **Real example:** A 500-movie library (5 TB H.264) shrinks to **2.5 TB HEVC** in 2-3 days with
  NVIDIA RTX 3060.
</Check>
...
<Card title="Maximum Storage Savings" icon="hard-drive">
  When disk space is expensive and you prioritize smallest file sizes over encoding speed. AV1
  achieves 10-20% better compression than HEVC.
</Card>

<Card title="Future-Proofing Library" icon="sparkles">
  AV1 is royalty-free and backed by Google, Netflix, Amazon. It's the long-term successor to HEVC
  with improving hardware support.
</Card>

<Card title="Chrome/Firefox Streaming" icon="chrome">
  Web-based media servers (Plex, Jellyfin, Emby) benefit from AV1's native support in Chrome/Firefox
  for efficient browser playback.
</Card>

<Card title="Archival Encodes" icon="archive">
  Encoding a library once for long-term storage where speed doesn't matter. AV1 provides best
  quality-per-byte for archival purposes.
</Card>
...
<Warning>
  **AV1 hardware encoding is rare.** Only Intel Arc GPUs (2022+) support AV1 encoding acceleration.
  Most users will encode on CPU at 1/10th the speed of HEVC.
</Warning>
...
<Check>
  **Real example:** A 500-movie library (5 TB H.264) shrinks to **2 TB AV1** but takes **2-3 weeks**
  to encode on Intel i9 CPU.
</Check>
...
Both [HEVC](/glossary#codec) and AV1 use **[CRF](/glossary#crf) (Constant Rate Factor)** to control quality. Lower CRF = higher quality + larger files.
...
<Info>
  **BitBonsai default: [CRF](/glossary#crf) 23** for both [HEVC](/glossary#codec) and AV1. This
  provides transparent quality indistinguishable from the source for most content.
</Info>
...
<Tip>
  **How to adjust [CRF](/glossary#crf):** Settings → Encoding → Quality → CRF slider. Lower for
  archival quality, higher for streaming/mobile [libraries](/glossary#library).
</Tip>
...
[Hardware encoding](/glossary#hardware-acceleration) uses [GPU](/glossary#gpu)/iGPU for 3-10x speedup vs CPU. Quality is slightly lower but visually transparent for most content.

<Note>
  **What is hardware transcoding?** Using your graphics card to encode videos 5-10x faster than
  CPU-only encoding. See [hardware acceleration](/glossary#hardware-acceleration) in the glossary.
</Note>
...
<Tip>
  **NVENC quality tip:** Use preset `p7` (slowest) for best quality. Only 10-20% slower than `p4`
...
<Check>
  **Intel Arc is the only consumer GPU with AV1 encoding.** If you want fast AV1 encodes, Arc
  A380/A750/A770 is your best option.
</Check>
...
<Warning>
  AMD hardware encoders have lower quality than NVIDIA/Intel. Recommended only for non-critical
  encodes or when NVENC/QSV unavailable.
</Warning>
...
<Warning>
  **AV1 smart TV support is extremely limited.** If you watch on TV, use HEVC for guaranteed
  hardware decoding. AV1 may software-decode (slow, stuttering).
</Warning>
...
<Info>
  **For web streaming (Plex, Jellyfin):** Use HEVC if users watch in Safari/Edge, AV1 if primarily
  Chrome/Firefox.
</Info>
