...
**CRF (Constant Rate Factor)** — a quality setting for encoding. Lower numbers = better quality but larger files.
...
**GPU (Graphics Processing Unit)** — a graphics card that can be used to accelerate video encoding. Using a GPU is **5-10x faster** than CPU-only encoding.

**Supported GPUs:**

- NVIDIA (GTX 1050+ or RTX series) — NVENC encoder
- Intel (6th gen Skylake+) — QuickSync (QSV) encoder
- AMD (RX 400 series+) — VCE/VCN encoder
...
The open-source tool BitBonsai uses internally to encode video. You don't interact with FFmpeg directly — BitBonsai handles it automatically.

**What it does:** Reads source video, applies codec conversion, writes output file.

**Why it matters:** Most error messages in logs come from FFmpeg. If you see `ffmpeg` in an error, check [Troubleshooting](/advanced/troubleshooting).

---

### Resolution

The dimensions of a video in pixels, expressed as width × height.

**Common resolutions:**

- **1080p** — 1920 × 1080 (Full HD, most common)
- **4K / 2160p** — 3840 × 2160 (Ultra HD, 4× larger than 1080p)
- **720p** — 1280 × 720 (HD)

**Encoding impact:** 4K files take ~4× longer to encode than 1080p at the same codec.
...
**Example:** `ghcr.io/bitbonsai/backend:latest` is the backend image name.
...
Running BitBonsai across multiple machines to encode videos in parallel. One **main node** coordinates work; additional **child nodes** (workers) share the load.

**How it works:** All nodes connect to the same PostgreSQL database and share video files via NFS.

**Related:** [Multi-Node Setup](/advanced/multi-node) | [Distributed Encoding Architecture](/advanced/distributed-encoding)
...
An additional computer (worker) that helps encode videos. Receives jobs from the main node, encodes them, and reports back.
...
**NFS (Network File System)** — a way for multiple computers to access the same files over a network. Required for multi-node setups.
...
**API (Application Programming Interface)** — the internal communication system BitBonsai uses. You don't interact with it directly - the web interface does.
...
**Engine:** PostgreSQL 15+ (required — BitBonsai does not support SQLite)
