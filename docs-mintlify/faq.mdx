...
<Accordion title="What is Zen Mode?">
Zen Mode is BitBonsai's default interface that hides power-user features for a cleaner experience. Most users never need Advanced Mode.

To switch modes: **Settings → Advanced Settings → Advanced Mode toggle**

See [Display Modes](/settings/modes) for details.
</Accordion>
