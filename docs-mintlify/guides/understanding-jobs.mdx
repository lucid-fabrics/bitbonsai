...
<Warning>
If jobs stay in TRANSFERRING for more than 30 seconds, your NFS mount may be degraded. Check NFS connectivity on all nodes. See [Multi-Node Setup](/advanced/multi-node) for NFS configuration requirements — shared storage is mandatory for multi-node encoding.
</Warning>
