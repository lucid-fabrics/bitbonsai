...
<Note>
**What is a job?** A job is a single video file being encoded. Each video in your library becomes one job. See [glossary](/glossary#job) for more details.
</Note>
...
### NEEDS_DECISION
- **What it means:** File requires user confirmation before encoding
- **Common reasons:**
  - **Codec already matches target** - Source file is already in the target codec (e.g., HEVC → HEVC)
  - **Container incompatibility** - Audio/video streams may not work with the target container
- **User action:** Review the issue and choose an action (Skip, Force Re-encode, or Cancel)
- **Where to find:** Queue → Filter by "Needs Decision" status

<Accordion title="Codec Already Matches Target">
When a file is already encoded in your target codec, re-encoding would waste time and potentially increase file size without improving quality.

**Options:**
- **Skip Encoding** (Recommended) - Mark as complete without re-encoding
- **Force Re-encode** - Re-encode anyway (useful for re-compression at different CRF)
- **Cancel Job** - Remove from queue

<Tip>
**Pro tip:** If you frequently want to re-encode files at a different quality setting (e.g., re-compress old HEVC files at higher CRF), enable **"Allow same codec re-encoding"** in your policy's Advanced Settings. This skips the confirmation and proceeds directly to encoding.
</Tip>
</Accordion>
...
### TRANSFERRING
- **What it means:** ([Multi-node](/glossary#multi-node) only) File is being copied to worker node
- **User action:** None (automatic)
- **Duration:** Depends on network speed and file size
- **Why this happens:** Some multi-node setups don't use shared storage ([NFS](/glossary#nfs)), so files must be transferred before encoding
- **Next step:** Encoding starts after transfer completes

<Warning>
If you see many TRANSFERRING jobs, consider setting up [NFS](/glossary#nfs) shared storage for instant zero-copy access. See [Multi-Node Setup](/advanced/multi-node).
</Warning>
...
<Tip>
**Slow encoding?** Check Settings → [Hardware Acceleration](/glossary#hardware-acceleration) and enable NVIDIA/Intel/AMD [GPU](/glossary#gpu) encoding if available. HEVC on GPU is 3-10x faster than CPU.
</Tip>
...
<Check>
**Example:** A 10 GB H.264 movie encoded to [HEVC](/glossary#codec) becomes ~5 GB (50% savings). No quality loss with default [CRF](/glossary#crf) settings.
</Check>
...
<Note>
Bulk actions are hidden by default. Enable **Advanced Mode** in Settings to access bulk retry, cancel all, and rebalance features. See [Display Modes](/settings/modes).
</Note>
