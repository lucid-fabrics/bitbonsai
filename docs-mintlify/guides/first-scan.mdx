...
- Docker volume mounts configured for video directories (see [Requirements](/installation/requirements))
...
<AccordionGroup>
  <Accordion title="Library scan finds 0 videos">
    ...
    **Solution:**
    - Verify path exists: `docker compose exec backend ls /media/movies`
  ...
  <Accordion title="Jobs stuck in QUEUED status">
    ...
    **Solution:**
    ...
    - Review worker logs: `docker compose logs backend`
...
<Warning>
  **Always backup your original files** before enabling auto-replace. While BitBonsai validates
  encoded output, hardware failures or power loss during replacement can cause data loss.
</Warning>
