...
<Step title="Search for BitBonsai">Search for `bitbonsai` and select the template.</Step>
...
<Step title="Configure Repository">``` Repository: ghcr.io/bitbonsai/backend:latest ```</Step>

<Step title="Set Network Mode">**Network Type:** `bridge` (recommended)</Step>

<Step title="Configure Ports">
  | Container Port | Host Port | Description | |----------------|-----------|-------------| | 3100 |
  3100 | Backend API |
</Step>

<Step title="Add Path Mappings">
  | Container Path | Host Path | Mode | Description |
  |----------------|-----------|------|-------------| | `/config` | `/mnt/user/appdata/bitbonsai` |
  Read/Write | App config and database | | `/media` | `/mnt/user/media` | Read/Write | Media
  libraries (adjust to your path) |
</Step>

  <Step title="Set Environment Variables">
    | Variable | Value | Description |
    |----------|-------|-------------|
    | `DATABASE_URL` | `postgresql://bitbonsai:changeme@[POSTGRES-CONTAINER-IP]:5432/bitbonsai` | PostgreSQL connection string (required) |
    ...
    <Warning>
    `DATABASE_URL` is required — the backend will crash on startup without it. Replace `[POSTGRES-CONTAINER-IP]` with your Unraid IP and use the same password as `POSTGRES_PASSWORD`.
    </Warning>
...
<Step title="Configure Repository">``` Repository: ghcr.io/bitbonsai/frontend:latest ```</Step>

<Step title="Set Network Mode">**Network Type:** `bridge`</Step>

<Step title="Configure Port">
  | Container Port | Host Port | Description | |----------------|-----------|-------------| | 4210 |
  4210 | Web interface |
</Step>
...
<Warning>
  **Critical:** When adding libraries in BitBonsai, use the **container path** (e.g.,
  `/videos/movies`), NOT the Unraid host path.
</Warning>

<Tip>
  **Best Practice:** Map `/mnt/user/` → `/mnt/user/` to avoid path translation issues. This allows
  BitBonsai to use identical paths to Unraid.
</Tip>
...
3. If files are not detected, check Docker logs:
   ```bash
   docker logs bitbonsai-backend  # or use Unraid Docker tab → container → Logs button
   ```
...
<Note>Bridge mode is recommended for security. Frontend and backend communicate via host IP.</Note>
...
<Step title="Restart Container">Stop and start the backend container.</Step>
...
<Warning>
  GPU encoding requires HEVC (h264_nvenc/hevc_nvenc) or AV1 (av1_nvenc) codec support. Check your
  GPU's capabilities.
</Warning>
...
3. Back up and recreate the DB if needed:

   ```bash
   # Backup first (replace CONTAINER_IP with your postgres container IP)
   docker exec -it bitbonsai-postgres pg_dump -U bitbonsai bitbonsai > backup.sql
   ```

   Then delete `/mnt/user/appdata/bitbonsai/postgres` and restart (will recreate DB from scratch)

   <Warning>
     Deleting the postgres data directory permanently removes all job history and settings. Backup
     first.
   </Warning>
