...
| RAM            | 4GB+ (8GB+ recommended) | 4 GB minimum for single job; 8 GB for parallel encoding |

<Note>
  BitBonsai requires access to your video library via volume mounts. Plan your directory structure
  before installation.
</Note>
...
| `NODE_MODE`       | Node role: `MAIN` (default) or `LINKED` (worker)        | MAIN       | No         |
| `MAIN_NODE_URL`   | URL of main node API (required when `NODE_MODE=LINKED`) | -          | Multi-node |
| `CONCURRENT_JOBS` | Max simultaneous encoding jobs per node                 | 1          | No         |
...
<Tip>
  For production deployments, use Docker secrets or environment files instead of hardcoding
  credentials.
</Tip>
...
```yaml
volumes:
  - /path/to/your/videos:/media # replace with your actual path
```
...
- /mnt/media:/media
  # BitBonsai will scan subdirectories: /media/movies, /media/tv, etc.
```
...
<Warning>
  Do not use `/tmp` on the host if it's a tmpfs (RAM disk). Encoding generates large temporary
  files.
</Warning>
...
<Steps>
  ...
  <Step title="Add Video Library">
    ...
    - **Name**: Movies
    - **Path**: `/media/movies` (relative to container mount)
    - **Type**: Movies or TV Shows

    <Tip>
    Use the `/media` prefix since that's the container mount point, not your host path.
    </Tip>
...
<Card title="Codec Selection" icon="gear" href="/advanced/codec-selection">
...
<Card title="Multi-Node Setup" icon="server" href="/advanced/multi-node">
  ...
  <Card title="Backup & Recovery" icon="database" href="/faq#how-do-i-back-up-my-database">
