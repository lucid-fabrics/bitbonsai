...
**NVIDIA Setup:**
```bash
# Install NVIDIA Container Toolkit (current method)
...
<Tip>
Only port **4210** needs to be accessible from your browser for normal use. Port 3100 (backend API) is also exposed for direct API access and multi-node child node connections. Port 5432 (PostgreSQL) is internal to Docker and should not be exposed to untrusted networks.
</Tip>
